...
class FileSaver {
    ...
    add(uploadedFile, containingFolder) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!uploadedFile)
                throw new Error("uploadedFile can't be empty.");
            if (!containingFolder)
                containingFolder = this.containingFolder;
            if (!fs_1.default.existsSync(containingFolder))
                fs_1.default.mkdirSync(containingFolder, { recursive: true });
            const lastDotIndex = uploadedFile.name.lastIndexOf(".");
            const extension = lastDotIndex === -1 ? "" : uploadedFile.name.substring(lastDotIndex);
            const fileName = (0, uuid_1.v4)() + extension;
            const absolutePath = path_1.default.join(containingFolder, fileName);
            yield uploadedFile.mv(absolutePath);
            return fileName;
        });
    }
    ...
    getFilePath(fileName, returnDefaultFileIfNotFound = true, containingFolder) {
        if (!containingFolder)
            containingFolder = this.containingFolder;
        let filePath = path_1.default.join(containingFolder, fileName);
        if (returnDefaultFileIfNotFound && !fs_1.default.existsSync(filePath))
            filePath = path_1.default.join(__dirname, "file-not-found.jpg");
        return filePath;
    }
    delay(ms) {
